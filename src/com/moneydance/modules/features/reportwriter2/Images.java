...
public class Images {
	...
	public Image saveImg = null; 
	public Image treeDBImg = null;
	public Image mathBookImg = null;
	public Image validateImg = null;
	public Image patternImg = null;

	public Images() {
		InputStream stream = getClass().getResourceAsStream(Constants.RESOURCES+"edit_node_32px.png");
	    	Main.rwDebugInst.debug("Images", "Images", MRBDebug.DETAILED, "loading  "+Constants.RESOURCES+"edit_node_32px.png");
		if (stream != null) {
				editImg = new Image(stream);
			    	Main.rwDebugInst.debug("Images", "Images", MRBDebug.DETAILED, "loaded "+Constants.RESOURCES+"edit_node_32px.png");
			try {
				stream.close();
			}
			catch (IOException e) {}
		...
		InputStream saveload = getClass().getResourceAsStream(Constants.RESOURCES+"icons8-save-32.png");
		if (saveload!= null) {
			saveImg = new Image(saveload);
			try {
				saveload.close();
			}
			catch (IOException e) {}
		}	
		InputStream treeDBload = getClass().getResourceAsStream(Constants.RESOURCES+"icons8-database-16.png");
		if (treeDBload!= null) {
			treeDBImg = new Image(treeDBload);
			try {
				treeDBload.close();
			}
			catch (IOException e) {}
		}
		InputStream mathBookload = getClass().getResourceAsStream(Constants.RESOURCES+"icons8-math-book-32.png");
		if (mathBookload!= null) {
			mathBookImg = new Image(mathBookload);
			try {
				mathBookload.close();
			}
			catch (IOException e) {}
		}
		InputStream validateload = getClass().getResourceAsStream(Constants.RESOURCES+"icons8-check-all-32.png");
		if (validateload!= null) {
			validateImg = new Image(validateload);
			try {
				validateload.close();
			}
			catch (IOException e) {}
		}
		InputStream patternload = getClass().getResourceAsStream(Constants.RESOURCES+"icons8-translate-text-32.png");
		if (patternload!= null) {
			patternImg = new Image(patternload);
			try {
				patternload.close();
			}
			catch (IOException e) {}
